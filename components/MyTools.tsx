  ...
  return (
    <div className="space-y-2">
      <div className="flex items-center justify-between">
        <h3 className="text-lg font-bold text-black">Quick Tools</h3>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        {state.myTools.map((tool) => {
          const completedItems = tool.subItems.filter(
            (item) => item.completed
          ).length;
          const progressPercentage =
            (completedItems / tool.subItems.length) * 100;

          return (
            <Card
              key={tool.id}
              className="bg-gradient-to-br from-blue-50 to-indigo-100 border border-blue-200 shadow-sm"
            >
              <CardContent className="p-4">
                ...
                <div className="mb-4">
                  <div className="flex items-center justify-between text-sm text-gray-600 mb-2">
                    <span>Progress</span>
                    <span>
                      {completedItems}/{tool.subItems.length} (
