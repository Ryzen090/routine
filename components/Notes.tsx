    ...
    <div className="space-y-6 bg-white min-h-screen pt-5 lg:pt-0 pb-20 lg:pb-0 px-4 lg:px-0">
      ...
      <div className="flex items-center justify-center">
        <div>
          ...
          <p className="text-sm lg:text-base text-gray-600">Daily motivation and goals</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 lg:gap-6">
        {state.notes.map((note) => (
          <Card key={note.id} className="bg-white border border-gray-200 shadow-sm">
              ...
              {note.subNotes.length > 0 && (
                <div className="space-y-2">
                  {note.subNotes.map((subNote) => (
                    <div key={subNote.id} className="flex items-center gap-3 p-2 bg-gray-50 rounded">
                      <Checkbox
                        checked={subNote.completed}
                        onCheckedChange={() => toggleSubNote(note.id, subNote.id)}
                      />
                      <span className={`flex-1 text-sm ${
                        subNote.completed ? 'line-through text-gray-500' : 'text-black'
                      }`}>
                        {subNote.content}
                      </span>
                    </div>
                  ...
                  <div className="mt-3 pt-2 border-t border-gray-200">
                    <div className="flex items-center gap-2 text-xs text-gray-500">
                      ...
                      <span>
                        {note.subNotes.filter(sub => sub.completed).length} of {note.subNotes.length} completed
                      </span>
                    </div>
                  </div>
                </div>
