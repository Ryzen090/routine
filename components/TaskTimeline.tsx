...
export function TaskTimeline() {
  ...
  const saveDailyStats = () => {
    ...
    // Get existing stats
    const existingStats = JSON.parse(localStorage.getItem('dailyTaskStats') || '[]');
    
    // Update or add today's stats
    const todayIndex = existingStats.findIndex((stat: any) => stat.date === today);
    const todayStats = {
      date: today,
      completedTasks,
      totalTasks,
  ...
  return (
    <div className="bg-white lg:mb-0">
      ...
      <div className="px-4 lg:px-6 space-y-3 lg:space-y-4">
        {todaysTasks.length === 0 ? (
          ...
          todaysTasks.map((task, index) => (
            <div
              key={task.id}
              className={`${taskBackgroundColors[index % taskBackgroundColors.length]} rounded-xl lg:rounded-2xl p-3 lg:p-4 transition-all duration-200 hover:shadow-md ${
                task.completed ? 'opacity-60' : ''
              }`}
            >
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3 lg:gap-4 flex-1">
                  <div className="text-xl lg:text-2xl xl:text-3xl">{task.icon}</div>
